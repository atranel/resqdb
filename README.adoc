...
I created two templates for the QASC reports. One is for baseline report and can be found in the `backgrouns\qasc_baseline.pptx` and the second one is for the comparison between Pre and Post phase. For this one I created template called `qasc_comparison.pptx` and it is A4 landscape format. Two slides are prepared as default. And unchangable frames are already there. 
