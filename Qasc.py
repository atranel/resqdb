...
class Qasc():
    ...
    def _create_table(self, slide, table_specs, title, trow, tcol, site_df=None, pre_df=None, post_df=None):
        ''' Generate table in the report. '''
        # Add table to the slide, we need table with 21 rows and 3 columns
        shape = first_slide.shapes.add_table(
            trow, #rows 
            tcol, # cols
            table_specs['left'], 
            table_specs['top'],
        ...
        # Set column width
        widths = [Cm(8.0), Cm(5.0), Cm(3.0), Cm(3.0)]
        ...
        table_title = title
        self._add_column_name(cell, table_title, alignment='center')
        ...
        columns = [str(site_df['n'].iloc[0]), '100']
        ...
        columns = [
            str(site_df[name].iloc[0]), 
            str(site_df[self._get_percentage_column_name(name)].iloc[0])]
        self._insert_values(table=table, row=nrow, values=columns)
    ...
    def generate_baseline_report(self, df=None):
        ...
        # Get site IDs from the statistics
        sites = set(df['SITE_ID'].tolist())
        sites = [x for x in sites if not x.endswith('_AUD')]
        sites = ['AM_001']
        for site in sites:
            # Filter dataframe for site
            site_df = df.loc[df['SITE_ID'] == site].copy()
            hospital_name = self.study_df.loc[self.study_df['unique_identifier'] == site, 'facility_name'].iloc[0]

            # Create output filename containing qasc, current date and site ID
            output_file = f'qasc_{site}_{datetime.now().strftime("%Y-%m-%d")}.pptx'
    ...
    def generate_pre_post_report(self, df, pre, post):
        ''' Generate report with pre/post comparison. '''
        stats = self.calculate_statistics(df=df)
        pre_stats = self.calculate_statistics(df=pre)
        post_stats = self.calculate_statistics(df=post)
        ...
        for site in sites:
            # Filter dataframe for site
            site_df = stats.loc[stats['SITE_ID'] == site].copy()
            pre_df = pre_stats.loc[pre_stats['SITE_ID'] == site].copy()
            post_df = post_stats.loc[post_stats['SITE_ID'] == site].copy()

            # Get hospital name based on study ID
            hospital_name = self.study_df.loc[self.study_df['unique_identifier'] == site, 'facility_name'].iloc[0]

            # Create output filename containing qasc, current date and site ID
            output_file = f'qasc_comp_{site}_{datetime.now().strftime("%Y-%m-%d")}.pptx'
            ...
            self._add_run(txBox, f'your hospital’s performance for the {pre_df['n'].iloc[0]} stroke patients you reviewed for the baseline audit XX/XX/XXXX and the {post_df['n'].iloc[0]} patients you reviewed during the post intervention period XX/XX/XXXX.', bold=True)
